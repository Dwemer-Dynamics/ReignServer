	...
	public class AssetPackage
	{
		...
		protected virtual void Load(BinaryReader stream, bool loadDataIntoMemory)
		{
			...
			for (int i = 0; i < resourceNum; i++)
			{
				...
				var metadataBytes = stream.ReadBytes(checked((int)metadataSize));
				try
				{
					using (var metadataReader = metadataBytes.CreateBinaryReader())
					{
						assetItem.ReadMetadata(metadataReader, (int)metadataSize);
					}
				}
				catch
				{
					// New Bannerlord releases sometimes extend metadata records before
					// public parsers catch up. Keep the rest of the package readable.
