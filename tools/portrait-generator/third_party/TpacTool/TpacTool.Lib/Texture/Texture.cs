...
namespace TpacTool.Lib
{
	public class Texture : AssetItem
	{
		...
		/// <summary>
		/// Dimensions of the largest mip level resident in the TPAC file.
		/// Current Bannerlord packages can stream higher mip levels separately.
		/// </summary>
		public int ResidentWidth { private set; get; }

		public int ResidentHeight { private set; get; }

		public int ResidentMipmapCount { private set; get; }
		...
		public override void ConsumeDataSegments(AbstractExternalLoader[] externalData)
		{
			foreach (var externalLoader in externalData)
			{
				...
				{
					int residentWidth = (int)Width;
					int residentHeight = (int)Height;
					int residentMipmaps = (int)MipmapCount;
					while (residentMipmaps > 1 &&
						GetMipChainByteCount(residentWidth, residentHeight, residentMipmaps, Format) * ArrayCount > pixelData._actualSize)
					{
						residentWidth = Math.Max(residentWidth >> 1, 1);
						residentHeight = Math.Max(residentHeight >> 1, 1);
						residentMipmaps--;
					}
					ResidentWidth = residentWidth;
					ResidentHeight = residentHeight;
					ResidentMipmapCount = residentMipmaps;
					var ud = pixelData.UserData;
					ud[TexturePixelData.KEY_WIDTH] = residentWidth;
					ud[TexturePixelData.KEY_HEIGHT] = residentHeight;
					ud[TexturePixelData.KEY_ARRAY] = (int) ArrayCount;
					ud[TexturePixelData.KEY_MIPMAP] = residentMipmaps;
		...
		private static ulong GetMipChainByteCount(int width, int height, int mipmapCount, TextureFormat format)
		{
			ulong total = 0;
			int alignment = format.GetAlignSize();
			int bitsPerPixel = format.GetBitsPerPixel();
			for (int mip = 0; mip < mipmapCount; mip++)
			{
				int alignedWidth = width + ((alignment - width % alignment) % alignment);
				int alignedHeight = height + ((alignment - height % alignment) % alignment);
				total += (ulong)alignedWidth * (ulong)alignedHeight * (ulong)bitsPerPixel / 8UL;
