...
1. Confirm the selected WSL distro and licensed Bannerlord directory. The standard local distro is `DwemerAI4Skyrim3`; resolve the game directory from the current machine. Verify the four pinned Bannerlord dependency modules against `ReignRelease/dependencies.lock.json`. Preserve differing existing modules. Do not launch the game.
