...
For source development, obtain the canonical validation plan/status, run the manifest-selected validation, then `ReignMcp/scripts/reign-validate.ps1 -LinuxServer -Restore -RequestingTaskId <task UUID>`. Use only its successful `reign-linux-build-v1` report. Sync reviewed server source to `/var/www/html/ReignServer`, preserving `.git` and all external runtime data, and run `sudo ddistro_reign prepare` for the pinned dependencies.
...
Verify public port 8089, internal server 5101, worker 5102, and fresh `/var/lib/dwemerdistro/reign/logs/server.log`. The worker must report a loaded 384-dimension embedding model. Stop/start only through `ddistro_reign`, and verify the owned worker stops too. Never stop MiniMe on 8082 or alter unrelated mod databases.

Data and secrets remain in `/var/lib/dwemerdistro/reign`; executables and dependencies are separate retained versions. Never run campaign/destructive/provider tests on active data. A process/health check is not in-game or native portrait-renderer proof. Report exact source, artifact, version, deployment, validation, and remaining limits.
