...
SELECT current_database(),current_user,
       ...
       (SELECT COALESCE(sum(reign_meta.get_schema_size(r.schema_name)),0)
        ...
        public static void RecordSqliteMigration(
            string migrationId,
            string campaignId,
            string sourcePath,
            long sourceBytes,
            ...
            string errorText)
        {
            ...
            using (NpgsqlConnection connection = OpenDatabaseConnection())
            using (NpgsqlCommand command = new NpgsqlCommand(@"
INSERT INTO reign_meta.migration_runs(
    migration_id,campaign_id,source_path,source_bytes,status,
    started_utc,completed_utc,source_table_count,
    destination_table_count,source_row_count,destination_row_count,
    validation_json,error_text)
VALUES(@migration_id,@campaign_id,@source_path,@source_bytes,@status,
       now(),now(),@source_table_count,@destination_table_count,
       @source_row_count,@destination_row_count,
       CAST(@validation_json AS jsonb),@error_text)
ON CONFLICT(migration_id) DO UPDATE
SET status=excluded.status,
    completed_utc=excluded.completed_utc,
    destination_table_count=excluded.destination_table_count,
    destination_row_count=excluded.destination_row_count,
    validation_json=excluded.validation_json,
    error_text=excluded.error_text;
UPDATE reign_meta.campaign_registry
SET migrated_from_sqlite_utc=CASE WHEN @status='completed' THEN now()
                                  ELSE migrated_from_sqlite_utc END,
    migration_state=@status,
    migration_report_json=CAST(@validation_json AS jsonb),
    updated_utc=now()
WHERE campaign_id=@campaign_id;", connection))
            {
                ...
                command.Parameters.AddWithValue(
                    "destination_table_count", destinationTableCount);
                command.Parameters.AddWithValue(
                ...
                command.Parameters.AddWithValue(
                    ...
                    string.IsNullOrWhiteSpace(validationJson)
                        ? "{}" : validationJson);
                command.Parameters.AddWithValue(
                    "error_text", errorText ?? string.Empty);
                command.ExecuteNonQuery();
            }
        }
