...
async function main() {
  ...
  const source = fs.readFileSync(path.join(root, 'ReignBetaServer/src/Modules/Platform/ReignXpOptions.cs'), 'utf8');
  const program = fs.readFileSync(path.join(root, 'ReignBetaServer/src/Modules/Platform/Program.cs'), 'utf8');
  const verbatim = (name) => {
  ...
  const start = program.indexOf('return @"<!doctype html>', program.indexOf('private static string ControlCenterHtml()'));
  const template = program.slice(start + 'return @"'.length, program.indexOf('</html>".Replace', start) + 7).replace(/""/g, '"');
    ...
    for (const width of [800, 1280, 1920]) {
      ...
      await context.route('**/*', async route => {
        ...
        if (request.method() === 'GET' && url.pathname === '/assets/ReignLogo.png')
          return route.fulfill({ contentType: 'image/png', body: fs.readFileSync(path.join(root, 'ReignBetaServer/assets/ReignLogo.png')) });
