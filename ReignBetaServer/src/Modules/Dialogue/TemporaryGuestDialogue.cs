    ...
    internal static partial class Program
    {
        ...
        private static Dictionary<string, object> TemporaryGuestDialogueContext(Dictionary<string, object> payload, string heroId)
        {
            ...
            if (!ReadBool(context, "enabled", false)
                ...
                || !new[] { "Active", "ReviewDue", "Departing", "Returning", "Completed", "HostileBanishmentPending" }.Contains(phase))
                return null;
            var projected = new Dictionary<string, object>();
            foreach (string key in new[] { "heroId", "agreementId", "purpose", "termKind", "phase", "returnSettlementId" })
            ...
            foreach (string key in new[] { "startedDay", "reviewDueDay", "returnDueDay", "observedWorldDay" })
            {
