...
ReignLiveTest.exe run verification_contracts\workspace\ReignBetaServer\ReignLiveTest\scenarios\spymaster-smoke.json --timeout 900
ReignLiveTest.exe run verification_contracts\workspace\ReignBetaServer\ReignLiveTest\scenarios\spymaster-feature.json --timeout 1800
```
...
```text
ReignLiveTest.exe run verification_contracts\workspace\ReignBetaServer\ReignLiveTest\scenarios\spymaster-save-prepare.json --timeout 900
...
ReignLiveTest.exe arm --minutes 120
ReignLiveTest.exe run verification_contracts\workspace\ReignBetaServer\ReignLiveTest\scenarios\spymaster-save-verify.json --timeout 900
```
...
Prepared on 2026-08-10 and first exercised in an actual Bannerlord campaign on 2026-08-11. The canonical ordered manifest is `ReignBetaServer/ReignLiveTest/scenarios/spymaster-organic-manifest.json`. This suite complements the deterministic matrix; it does not replace it. A completed run is established only by its machine-readable run artifacts and final acceptance report, not by this document.
...
Arm the already-visible server bridge, run each JSON from the installed `verification_contracts/workspace/ReignBetaServer/ReignLiveTest/scenarios` directory, and retain every resulting run ID. The intended order is:
...
ReignLiveTest.exe run verification_contracts\workspace\ReignBetaServer\ReignLiveTest\scenarios\spymaster-organic-preflight.json --timeout 1800
```
