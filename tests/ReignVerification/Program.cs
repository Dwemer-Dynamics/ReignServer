...
namespace ReignVerification
{
    internal static class Program
    {
        private static int Main(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("ReignServer.dll could not be found. Set REIGN_SERVER_DLL or build ReignBetaServer first.");
                return 2;
            }
        ...
        private static string FindServerExecutable()
        {
            ...
            for (int i = 0; cursor != null && i < 8; i++, cursor = cursor.Parent)
            {
                string[] candidates =
                {
                    Path.Combine(cursor.FullName, "ReignBeta", "server", "app", "ReignServer.dll"),
                    Path.Combine(cursor.FullName, "ReignBetaServer", "bin", "Release", "ReignServer.dll"),
                    Path.Combine(cursor.FullName, "ReignServer.dll")
                };
