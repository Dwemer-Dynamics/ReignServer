...
namespace ReignBetaServer
{
    internal static partial class Program
    {
            ...
            bool includeExpensiveSubsystems)
        {
            ...
            string serverRoot = Path.Combine(root, "ReignServer");
            string controlCenterMarkup = File.ReadAllText(Path.Combine(serverRoot, "ui", "index.html"));
            ...
            var chatComposerBindings = new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ["ReignCastleChatScreen.xml"] = "@InputText",
                ["ReignCorrespondenceScreen.xml"] = "@InputText",
                ["ReignCourtPetitionScreen.xml"] = "@InputText",
                ["ReignGovernmentScreen.xml"] = "@ComposerText",
                ["ReignIndividualChatScreen.xml"] = "@InputText",
                ["ReignPartyChatScreen.xml"] = "@InputText",
                ["ReignRoyalCouncilScreen.xml"] = "@InputText",
                ["ReignSocialEventScreen.xml"] = "@InputText",
                ["ReignTavernHouseScreen.xml"] = "@InputText",
                ["ReignWarCouncilScreen.xml"] = "@MessengerInputText"
            };
            var boundedChatComposers = new List<string>();
            foreach (KeyValuePair<string, string> composer in chatComposerBindings)
            {
                string prefabPath = Path.Combine(guiRoot, "Prefabs", composer.Key);
                XmlDocument document = TryParseVerificationXml(File.Exists(prefabPath) ? File.ReadAllText(prefabPath) : "");
                XmlElement input = document?.SelectSingleNode("//EditableTextWidget[@Text='" + composer.Value + "']") as XmlElement;
                if (input == null || !VerificationXmlAttributeEquals(input, "MaxLength", "2147483647"))
                    boundedChatComposers.Add(composer.Key);
            }
            string governmentVmInputPath = VerificationSourceLocator.ResolveUnique(clientRoot, "ReignGovernmentScreenVM.cs", "src");
            string governmentHearingInputPath = VerificationSourceLocator.ResolveUnique(clientRoot, "ReignGovernmentHearingDialogue.cs", "src");
            string courtLifeInputPath = VerificationSourceLocator.ResolveUnique(serverRoot, "CourtLifeDialogue.cs", "src");
            string royalCouncilInputPath = VerificationSourceLocator.ResolveUnique(serverRoot, "RoyalCouncil.cs", "src");
            string rulerDocketInputPath = VerificationSourceLocator.ResolveUnique(serverRoot, "RulerDocketDialogue.cs", "src");
            string correspondenceInputPath = VerificationSourceLocator.ResolveUnique(serverRoot, "Relationships.cs", "src", "Modules", "Relationships");
            string governmentVmInput = File.Exists(governmentVmInputPath) ? File.ReadAllText(governmentVmInputPath) : "";
            string governmentHearingInput = File.Exists(governmentHearingInputPath) ? File.ReadAllText(governmentHearingInputPath) : "";
            string courtLifeInput = File.Exists(courtLifeInputPath) ? File.ReadAllText(courtLifeInputPath) : "";
            string royalCouncilInput = File.Exists(royalCouncilInputPath) ? File.ReadAllText(royalCouncilInputPath) : "";
            string rulerDocketInput = File.Exists(rulerDocketInputPath) ? File.ReadAllText(rulerDocketInputPath) : "";
            string correspondenceInput = File.Exists(correspondenceInputPath) ? File.ReadAllText(correspondenceInputPath) : "";
            bool fullChatInputPreserved = !governmentVmInput.Contains("message.Length > 1200", StringComparison.Ordinal)
                && !governmentHearingInput.Contains("playerText.Length > 4000", StringComparison.Ordinal)
                && !governmentHearingInput.Contains("boundedMessage.Length > 2000", StringComparison.Ordinal)
                && !courtLifeInput.Contains("LimitText(ReadString(payload, \"playerText\", \"\"), 2000)", StringComparison.Ordinal)
                && !royalCouncilInput.Contains("LimitText(ReadString(payload, \"playerText\", \"\"), 1200)", StringComparison.Ordinal)
                && !rulerDocketInput.Contains("LimitText(ReadString(payload, \"playerText\", \"\"), 800)", StringComparison.Ordinal)
                && !correspondenceInput.Contains("if (body.Length > 4000) body = body.Substring(0, 4000);", StringComparison.Ordinal);
            bool unboundedChatInputContract = boundedChatComposers.Count == 0 && fullChatInputPreserved;
            AddVerificationCheck(checks, "contracts.chat_composers_preserve_full_input", "ui_contracts",
                unboundedChatInputContract,
                unboundedChatInputContract
                    ? "Every player-facing Reign chat composer bypasses Gauntlet's default text cap, and specialized conversation routes preserve the complete submitted text."
                    : "A player-facing chat composer or specialized conversation route still caps or truncates submitted text.",
                new Dictionary<string, object>
                {
                    ["checkedPrefabs"] = chatComposerBindings.Keys.ToArray(),
                    ["boundedPrefabs"] = boundedChatComposers,
                    ["fullInputRouting"] = fullChatInputPreserved
                });
            ...
            string liveTestProgramPath = Path.Combine(root, "ReignServer", "tests", "ReignLiveTest", "Program.cs");
            ...
            AddVerificationCheck(checks, "contracts.social_event_readable_transcript", "ui_contracts", socialEventTranscriptReadable,
                socialEventTranscriptReadable
                    ? "Both Social and Wilderness Event transcripts top-align speaker names with wrapped dialogue, separate turns by 12 pixels, and preserve the approved gold-speaker/gray-body typography roles."
                    : "A Social or Wilderness Event transcript can compress wrapped dialogue, misalign speaker names, or lose turn separation.",
                new Dictionary<string, object> { ["prefab"] = socialEventPrefab,
                    ["socialTemplateValid"] = VerificationModernChatListTemplateMatches(socialEventChatListElement, "34", "12", "28"),
                    ["wildernessTemplateValid"] = VerificationModernChatListTemplateMatches(wildernessEventChatListElement, "42", "12", "32") });
            ...
            bool worldTestRumorInspectorValid = controlCenterMarkup.Contains("worldTestRumorInspector")
                && controlCenterMarkup.Contains("loadWorldTestRumorInspector")
                && controlCenterMarkup.Contains("selectWorldTestRumor")
                && controlCenterMarkup.Contains("subsystem','rumor_subjects")
                && controlCenterMarkup.Contains("worldTestRumorId")
                && controlCenterMarkup.Contains("row?.occurrence_id")
                && controlCenterMarkup.Contains("row?.provenance_summary")
                && controlCenterMarkup.Contains("row?.created_day??row?.world_day")
                && controlCenterMarkup.Contains("Subjects and reputation tags");
            ...
            Match worldTestSectionMatch = Regex.Match(controlCenterMarkup,
            ...
            bool worldTestObservatoryUiValid = worldTestObservatoryIds.All(id =>
                    controlCenterMarkup.Contains("id='" + id + "'"))
            ...
            string liveTestControllerPath = Path.Combine(serverRoot, "tests", "ReignLiveTest", "Program.cs");
            ...
            string derivativeCorePath = Path.Combine(root, "ReignServer", "shared", "Reign.Shared.Source", "Core", "ReignPortraitDerivativeCore.cs");
            ...
            string controlCenterSource = mbtiProgramSource + controlCenterMarkup;
			...
			string pngReencodePath = VerificationSourceLocator.ResolveUnique(Path.Combine(root, "ReignServer", "shared", "Reign.Shared.Source"), "PngReencode.cs", "Portraits");
			string pngReencodeText = File.Exists(pngReencodePath) ? File.ReadAllText(pngReencodePath) : "";
			string pngEncoderPath = VerificationSourceLocator.ResolveUnique(Path.Combine(root, "ReignServer", "shared", "Reign.Shared.Source"), "PngEncoder.cs", "Portraits");
            ...
            string nativeGeneratorRoot = Path.Combine(root, "ReignServer", "tools", "portrait-generator");
            ...
            string serverProjectContractPath = Path.Combine(serverRoot, "ReignServer.csproj");
            ...
            string sharedCompositionContractPath = Path.Combine(root, "ReignServer", "shared", "Reign.Shared.Source", "Core", "ReignPortraitDerivativeCore.cs");
            ...
            bool sharedPortraitCompositionValid = nativePortraitSourceText.Contains("return PortraitDerivativeCore.PortraitCompositionError(focus);")
                ...
                && serverProjectContractText.Replace('\\', '/').Contains("Compile Include=\"shared/Reign.Shared.Source/**/*.cs\"")
                && clientProjectContractText.Replace('\\', '/').Contains("Compile Include=\"$(ReignServerRoot)/shared/Reign.Shared.Source/**/*.cs\"");
            ...
            string subModulePath = VerificationSourceLocator.ResolveUnique(clientRoot, "SubModule.cs", "src/Modules/Platform");
            ...
            string serverProject = Path.Combine(serverRoot, "ReignServer.csproj");
            ...
            string serverAssembly = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ReignServer.dll");
        ...
        private static void RunProductionPipelineChecks(List<Dictionary<string, object>> checks, string requestedSuite, bool failFast)
        {
            ...
            foreach (Dictionary<string, object> testCase in cases)
            {
                ...
                {
                    Dictionary<string, object> failure = CompactVerificationFailure(row);
                    string assertion = ReadString(ReadDictionary(row, "assertion"), "summary", "");
                    if (!string.IsNullOrWhiteSpace(assertion))
                        failure["summary"] = ReadString(failure, "summary", "") + " " + LimitText(assertion, 500);
        ...
        private static Dictionary<string, object> BuildVerificationCoverageManifest()
        {
            ...
            string serverRoot = string.IsNullOrWhiteSpace(root) ? "" : Path.Combine(root, "ReignServer");
            ...
            string subModulePath = string.IsNullOrWhiteSpace(clientRoot) ? "" : VerificationSourceLocator.ResolveUnique(clientRoot, "SubModule.cs", "src/Modules/Platform");
        ...
        private static string FindVerificationSourceRoot()
        {
            ...
            if (Directory.Exists(Path.Combine(packagedContracts, "ReignBeta")) && Directory.Exists(Path.Combine(packagedContracts, "ReignServer"))) return packagedContracts;
            string configured = Environment.GetEnvironmentVariable("REIGN_SOURCE_ROOT") ?? "";
            if (!string.IsNullOrWhiteSpace(configured) && Directory.Exists(Path.Combine(configured, "ReignBeta")) && Directory.Exists(Path.Combine(configured, "ReignServer"))) return Path.GetFullPath(configured);
            Dictionary<string, object> settings = LoadSettings();
            ...
            {
                if (Directory.Exists(Path.Combine(cursor.FullName, "ReignBeta")) && Directory.Exists(Path.Combine(cursor.FullName, "ReignServer"))) return cursor.FullName;
            }
            return "";
        }
            ...
            string textMinHeight)
        {
            ...
            return row != null
                ...
                && VerificationXmlAttributeEquals(speaker, "HeightSizePolicy", "Fixed")
                && VerificationXmlAttributeEquals(speaker, "SuggestedWidth", "112")
                && VerificationXmlAttributeEquals(speaker, "SuggestedHeight", textMinHeight)
                && VerificationXmlAttributeEquals(speaker, "VerticalAlignment", "Top")
