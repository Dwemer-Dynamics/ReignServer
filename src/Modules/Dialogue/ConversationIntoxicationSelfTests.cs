...
namespace ReignBetaServer
{
    internal static partial class Program
    {
        private static List<Dictionary<string, object>> RunConversationIntoxicationSelfTests()
        {
            ...
            check("generic_drink_without_metadata_defaults_quarter", amount(omitted("*She takes a drink from her own cup of wine.*",
                new string[0])) == .25);
            check("generic_drink_respects_bounded_model_sip", amount(response("She takes a drink from her own cup of wine.", "sip", 1)) == .25);
            check("explicit_full_drink_remains_one", amount(response("She takes a full drink of wine.", "sip", 1)) == 1);
